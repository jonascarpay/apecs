...
newShape spacePtr' bodyPtr shape (fromIntegral -> ety) = withForeignPtr spacePtr' (go shape)
  where
    go (Convex [fmap realToFrac -> V2 x y] (realToFrac -> radius)) spacePtr =
      [C.block| cpShape* {
...
destroyShape spacePtr shapePtr = withForeignPtr spacePtr $ \space ->
  [C.block| void {
...
setSensor shape (fromIntegral . fromEnum -> isSensor) =
  [C.exp| void {
...
setElasticity shape (realToFrac -> elasticity) =
  [C.exp| void {
...
setDensity shape (realToFrac -> density) =
  [C.exp| void {
...
setFriction shape (realToFrac -> friction) =
  [C.exp| void {
...
setSurfaceVelocity shape (V2 (realToFrac -> x) (realToFrac -> y)) =
  [C.block| void {
  const cpVect vec = { $(double x), $(double y) };
