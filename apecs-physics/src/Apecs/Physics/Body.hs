...
newBody spacePtr (fromIntegral -> ety) = withForeignPtr spacePtr $ \space ->
  [C.block| cpBody* {
...
destroyBody spacePtr bodyPtr = withForeignPtr spacePtr $ \space ->
  [C.block| void {
...
setPosition bodyPtr (V2 (realToFrac -> x) (realToFrac -> y)) =
  [C.block| void {
...
setVelocity bodyPtr (V2 (realToFrac -> x) (realToFrac -> y)) =
  [C.block| void {
...
setAngle bodyPtr (realToFrac -> angle) =
  [C.block| void {
  cpBody *body = $(cpBody* bodyPtr);
...
setAngularVelocity bodyPtr (realToFrac -> angle) =
  [C.block| void {
  cpBody *body = $(cpBody* bodyPtr);
...
setForce bodyPtr (V2 (realToFrac -> x) (realToFrac -> y)) =
  [C.block| void {
...
applyForceAtLocalPoint bodyPtr force bPos =
  [C.block| void {
...
applyForceAtWorldPoint bodyPtr force wPos =
  [C.block| void {
  const cpVect frc = { $(double fx), $(double fy) };
  const cpVect pos = { $(double px), $(double py) };
...
setBodyMass bodyPtr (realToFrac -> angle) =
  [C.block| void {
  cpBody *body = $(cpBody* bodyPtr);
...
setMoment bodyPtr (realToFrac -> angle) =
  [C.block| void {
  cpBody *body = $(cpBody* bodyPtr);
...
setTorque bodyPtr (realToFrac -> angle) =
  [C.block| void {
  cpBody *body = $(cpBody* bodyPtr);
...
setCenterOfGravity bodyPtr (V2 (realToFrac -> x) (realToFrac -> y)) =
  [C.block| void {
  const cpVect vel = { $(double x), $(double y) };
