...
type SystemT w m a = StateT w m a

type System w a = StateT w IO a
...
instance {-# OVERLAPPABLE #-} (Functor m, HasStore w c, Get (GetStore w c) m c) => Get w m c where
  {-# INLINE exists #-}
  exists p ety = focusStore @c $ exists p ety
  {-# INLINE get #-}
  get ety = focusStore @c $ get ety

instance {-# OVERLAPPABLE #-} (Functor m, HasStore w c, Set (GetStore w c) m c) => Set w m c where
  {-# INLINE set #-}
  set a ety = focusStore @c $ set a ety

instance {-# OVERLAPPABLE #-} (Functor m, HasStore w c, Destroy (GetStore w c) m c) => Destroy w m c where
  {-# INLINE destroy #-}
  destroy p ety = focusStore @c $ destroy p ety

instance {-# OVERLAPPABLE #-} (Functor m, HasStore w c, Members (GetStore w c) m c) => Members w m c where
  {-# INLINE members #-}
...
instance Monad m => GGet w m U1 where
  {-# INLINE gexists #-}
  gexists _ _ = pure True
  {-# INLINE gget #-}
  gget _ = pure U1

instance Monad m => GGet w m V1 where
  {-# INLINE gexists #-}
...
instance (Monad m, GSet w m l, GSet w m r) => GSet w m (l :*: r) where
  {-# INLINE gset #-}
...
instance (Monad m, GDestroy w m l, GDestroy w m r) => GDestroy w m (l :*: r) where
  {-# INLINE gdestroy #-}
  gdestroy _ ety = gdestroy (Proxy @l) ety *> gdestroy (Proxy @r) ety

instance Monad m => GDestroy w m V1 where
  {-# INLINE gdestroy #-}
  gdestroy _ _ = pure ()
...
instance (Monad m, GMembers w m l, GMembers w m r) => GMembers w m (l :+: r) where
  {-# INLINE gmembers #-}
  gmembers _ =
    liftA2
...
instance Monad m => GMembers w m V1 where
  {-# INLINE gmembers #-}
...
--- ()
instance Monad m => Get w m ()

instance Monad m => Set w m () where set _ _ = pure ()
...
--- Entity
instance Monad m => Get w m Entity where
  {-# INLINE get #-}
  get ety = pure ety
  {-# INLINE exists #-}
...
instance (Monad m, Get w m c) => Get w m (Not c) where
  {-# INLINE exists #-}
...
--- Tuples
instance (Get w m a, Get w m b, Monad m) => Get w m (a, b)
instance (Set w m a, Set w m b, Monad m) => Set w m (a, b)
instance (Destroy w m a, Destroy w m b, Monad m) => Destroy w m (a, b)
instance (Members w m a, Get w m b, Monad m) => Members w m (a, b)
instance (Initialize m a, Initialize m b, Monad m) => Initialize m (a, b)

instance (Get w m a, Get w m b, Get w m c, Monad m) => Get w m (a, b, c)
instance (Set w m a, Set w m b, Set w m c, Monad m) => Set w m (a, b, c)
instance (Destroy w m a, Destroy w m b, Destroy w m c, Monad m) => Destroy w m (a, b, c)
instance (Members w m a, Get w m b, Get w m c, Monad m) => Members w m (a, b, c)
instance (Initialize m a, Initialize m b, Initialize m c, Monad m) => Initialize m (a, b, c)

instance (Get w m a, Get w m b, Get w m c, Get w m d, Monad m) => Get w m (a, b, c, d)
instance (Set w m a, Set w m b, Set w m c, Set w m d, Monad m) => Set w m (a, b, c, d)
instance (Destroy w m a, Destroy w m b, Destroy w m c, Destroy w m d, Monad m) => Destroy w m (a, b, c, d)
instance (Members w m a, Get w m b, Get w m c, Get w m d, Monad m) => Members w m (a, b, c, d)
instance (Initialize m a, Initialize m b, Initialize m c, Initialize m d, Monad m) => Initialize m (a, b, c, d)

instance (Get w m a, Get w m b, Get w m c, Get w m d, Get w m e, Monad m) => Get w m (a, b, c, d, e)
instance (Set w m a, Set w m b, Set w m c, Set w m d, Set w m e, Monad m) => Set w m (a, b, c, d, e)
instance (Destroy w m a, Destroy w m b, Destroy w m c, Destroy w m d, Destroy w m e, Monad m) => Destroy w m (a, b, c, d, e)
instance (Members w m a, Get w m b, Get w m c, Get w m d, Get w m e, Monad m) => Members w m (a, b, c, d, e)
instance (Initialize m a, Initialize m b, Initialize m c, Initialize m d, Initialize m e, Monad m) => Initialize m (a, b, c, d, e)

instance (Get w m a, Get w m b, Get w m c, Get w m d, Get w m e, Get w m f, Monad m) => Get w m (a, b, c, d, e, f)
instance (Set w m a, Set w m b, Set w m c, Set w m d, Set w m e, Set w m f, Monad m) => Set w m (a, b, c, d, e, f)
instance (Destroy w m a, Destroy w m b, Destroy w m c, Destroy w m d, Destroy w m e, Destroy w m f, Monad m) => Destroy w m (a, b, c, d, e, f)
instance (Members w m a, Get w m b, Get w m c, Get w m d, Get w m e, Get w m f, Monad m) => Members w m (a, b, c, d, e, f)
instance (Initialize m a, Initialize m b, Initialize m c, Initialize m d, Initialize m e, Initialize m f, Monad m) => Initialize m (a, b, c, d, e, f)

instance (Get w m a, Get w m b, Get w m c, Get w m d, Get w m e, Get w m f, Get w m g, Monad m) => Get w m (a, b, c, d, e, f, g)
instance (Set w m a, Set w m b, Set w m c, Set w m d, Set w m e, Set w m f, Set w m g, Monad m) => Set w m (a, b, c, d, e, f, g)
instance (Destroy w m a, Destroy w m b, Destroy w m c, Destroy w m d, Destroy w m e, Destroy w m f, Destroy w m g, Monad m) => Destroy w m (a, b, c, d, e, f, g)
instance (Members w m a, Get w m b, Get w m c, Get w m d, Get w m e, Get w m f, Get w m g, Monad m) => Members w m (a, b, c, d, e, f, g)
instance (Initialize m a, Initialize m b, Initialize m c, Initialize m d, Initialize m e, Initialize m f, Initialize m g, Monad m) => Initialize m (a, b, c, d, e, f, g)
