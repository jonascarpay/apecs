...
--
-- Takes the world's entity member set and a tag-getter function (typically
-- generated by @makeTaggedComponents@), and returns a map from tag sets to
-- entity counts. Uses 'IntSet' internally for compact tag set representation.
countCombinations
  :: (Monad m, Enum tag, Ord tag)
  => IS.IntSet                        -- ^ Entity IDs to census
  -> (Entity -> SystemT w m [tag])    -- ^ Tag getter (e.g. @getWorldEnumerableTags@)
  -> SystemT w m (M.Map (S.Set tag) Int)
countCombinations entities getTags = do
