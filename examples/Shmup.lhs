...
It is neither fast nor powerful, but its simplicity makes it perfect for simple games like these.
...
Let's start by defining some components.
We do so by first defining a data type for our component, and then give it an instance of `Component`.
The instance specifies the data structure we store a component in, in these cases `Map c`.
Stores change both the behaviour and performance characteristics of a component.
We will only look at stores that change the behaviour here, refer to the benchmark lhs files to see how they can impact performance.
...
Position and Velocity are straightforward; they define an entity's position and velocity.
...
This means that we can use both `get 0` and `get 123` to retrieve a global component.
We generally make it explicit that we're reading a global by using the entity `global`, as we will see later.
...
In this case, I chose to split as it makes it easier to iterate over e.g. just the Targets.
...
The exact entity argument does not matter for a global component, so we can use 0 in this case.
As we will see later, we usually make this explicit by using the `global` entity.
...
For example, we don't really need to read Velocity, we are only interested in removing it.
Furthermore, we don't want to write all three components, we just want to be able to delete them.

The next system illustrates how we can solve this issue, all from `cmap`.
...
If you've never seen it, the `Not @c` is from the TypeApplication pragma, and is equivalent to `Not :: Not c`.
...
If you have any questions, feel free to make an Issue, or ask me on twitter/reddit.
